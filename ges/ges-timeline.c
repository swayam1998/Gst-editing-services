 ...
 *
 * The configuration for timecodes in a timeline. The framerate is
 *
 ...
 * scale, meaning that the frame based API of #GESTimelineElement is
 * activated once this value is set.
 *
 * This has several implications, for example, if you set this value to `30/1`,
 * meaning 30 frames per second, then when you use:
 *
 * ```
 * ges_timeline_element_set_start (some_clip, GST_SECOND / 40)
 * ```
 *
 * `some_clip::start` property will be set to the start time of the frame right before
 * `GST_SECOND / 40 = 25000000ns`, knowing that the start of the frame number 1 (second frame)
 * at 30/1 fps is `GST_SECOND / 30 ~= 33333333`, the some_clip_start will be set to 0.
 *
