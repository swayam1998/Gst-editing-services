...
ges_timeline_element_class_init (GESTimelineElementClass * klass)
{
  ...
  properties[PROP_START] = g_param_spec_uint64 ("start", "Start",
      "The position in the timeline", 0, G_MAXUINT64,
      0, G_PARAM_READWRITE);
 ...
 * in number of frames
 * @frame: The number of the frame the element should start at
 * 
 ...
 * it is.
 * 
 * Returns: %TRUE if @frame could be set, %FALSE otherwise
 * 
 * Since: 1.18
 */
gboolean
 ...
 * in number of frames
 * @frame: The number of the first frame the element will output from
 ...
 * 
 * In the case of a video file, @frame is the frame number of the
 ...
 * in number of frames
 * @frame: The number of the frames the element lasts inside the timeline
 * 
 * Sets the number of frames that @self will last in the timeline,
 * The scale is the #GESTimeline:framerate of the timeline inside which
 * it is.
 * 
 * Returns: %TRUE if @frame could be set, %FALSE otherwise
 * 
 ...
 * in number of frames
 * @frame: The maximum number of frames the element can last inside a
 * timeline
 * 
    ...
    GESFrameNumber frames)
{
  ...
  switch (mode) {
    case GES_EDIT_MODE_TRIM:
    {
      ...
      GST_ERROR ("Layer prio offset: %ld", layer_prio_off);
